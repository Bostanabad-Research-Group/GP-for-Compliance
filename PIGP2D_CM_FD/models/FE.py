...
class FE:
        ...
        if(self.physics == self.PHYSICS_OPTIONS['Structural']):
            for elx in range(self.nelx):
                for ely in range(self.nely):
                    ...
                    self.edofMat[el,:]=np.array([2*n2+2, 2*n2+3, 2*n1+2, 2*n1+3, 2*n1, 2*n1+1, 2*n2, 2*n2+1])
        else:
   ...
   #-----------------------#
    ...
    def solve(self, density):
        ...
        sK = ((self.KE.flatten()[np.newaxis]).T * (1e-5 + density ** self.penal * (self.Emax - 1e-5))).flatten(order='F')
        K = coo_matrix((sK, (self.iK, self.jK)), shape=(self.ndof, self.ndof)).tocsc()
        K = self.deleterowcol(K, self.fixed, self.fixed).tocoo()
    
        # Replace np.int with int
        K = cvxopt.spmatrix(K.data, K.row.astype(int), K.col.astype(int))  # Use built-in int type
